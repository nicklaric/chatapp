...
function initApp(firebase) {
  ...
  console.log("[AppJS] Firebase services object:", Object.keys(firebase));
  ...
  console.log("[AppJS] Destructured Firebase services:");
  console.log("- auth:", !!auth);
  console.log("- db:", !!db);
  console.log("- deleteDoc:", !!deleteDoc);
  console.log("- updateDoc:", !!updateDoc);
  
  // Special check for deleteDoc function
  if (!deleteDoc) {
    console.error("[AppJS] deleteDoc is not available in the Firebase services! Using updateDoc workaround.");
    // Create a workaround function
    window.workaroundDeleteDoc = async function(docRef) {
      console.log("[AppJS] Using workaround deleteDoc with updateDoc");
    ...
    window.workaroundDeleteDoc = deleteDoc;
    console.log("[AppJS] deleteDoc is available, using the real function.");
  }
      ...
      for (const aiRole of aiParticipants) {
        try {
          ...
          try {
            ...
            // Use our workaround function instead of deleteDoc directly
            console.log(`[AppJS] Removing typing indicator with workaround function`);
            const typingDocRef = doc(db, "chats", currentChatId, "messages", typingMessageRef.id);
            await window.workaroundDeleteDoc(typingDocRef);
            ...
            // If there's an error generating the response, delete the typing indicator
            console.error(`[AppJS] Error generating ${aiRole} response:`, error);
            
            try {
              console.log(`[AppJS] Removing typing indicator with workaround function due to error`);
              const typingDocRef = doc(db, "chats", currentChatId, "messages", typingMessageRef.id);
              await window.workaroundDeleteDoc(typingDocRef);
              console.log(`[AppJS] Removed typing indicator due to error`);
            } catch (deleteError) {
              console.error(`[AppJS] Error removing typing indicator:`, deleteError);
            }
          }
        } catch (error) {
...
Your response:`;
      ...
      if (!responseDoc || !responseDoc.exists() || !responseDoc.data().response) {
        console.error(`[AppJS] Failed to get AI response after ${maxRetries} seconds`);
        throw new Error("Failed to get AI response in time");
      }
